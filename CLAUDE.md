...
**number**. Three new things, all sharing ONE registry so they can never disagree:
- **`app/kit/kit.js` + `app/kit/kit.html`** — a live kitchen sink at
  **`/app/__kit`**, served only when `FTZ_KIT=1` (otherwise the route falls
  through to the SPA and 404s). 18 components · 41 states · 5 groups.
  ⚠️ **It runs the REAL helpers**, pulled out of `app/app.js` at load time by
  `_kitExtract` — a kit carrying its own copy of a component drifts from the app
  within a week, and then it is worse than no kit. Rename a helper and the kit
  draws a loud red `.kit-miss` card; **a silent kit is a lie.**
  - ⚠️ `_kitDeclName` must **capture** the identifier, not strip the tail off the
    declaration. `const _FA_ICON_PATHS = {` minus its brace leaves
    `_FA_ICON_PATHS =`, which lands in the returned object literal as
    `_FA_ICON_PATHS =,` and takes the whole bag down with one "Unexpected token".
  - ⚠️ An async IIFE that rejects does **not** fire `pageerror`. The kit boot
    failed completely silently until it was made to report its own failures.
  - The kit **found its own first bug**: the component notes are prose that talks
    about markup (one says *never `<i class="fa-…">`*), and injecting them raw put
    113 real `<i class="fa-">` elements plus a native colour input on the page.
    `kitEsc()` now escapes them. The self-check is scoped to `.kit-c-b` so page
    chrome can never register as a component.
- **`docs/design-system.md` + `docs/design-system/*.png`** — generated by
  `node tools/gen-design-system.mjs`. **Do not hand-edit it**; change the registry
  and re-run. It **exits 1** when the self-check is dirty, so the doc cannot be
  regenerated while a component breaks a rule the doc states.
  **If a component is not on that page, it does not exist.**
- **Two guards**, `tools/check-icons.mjs` and `tools/check-tokens.mjs`.

**The measured debt, as of this stamp** (these are the numbers each phase brings
down; `tools/token-baseline.json` holds the ratchet and **may only ever go down**):

| | count |
| --- | --- |
| FA CSS-class icon sites (`<i class="fa-…">`) | **322** across 113 glyphs |
| …convertible right now (path already in `_FA_ICON_PATHS`) | **25** |
| Emoji standing in for an icon | **114** |
| Hardcoded colour literals outside `:root` | **3,226** (706 hex · 636 neutral · 1,884 rgb) |
| `font-weight:800/900` (the cap is 700) | **464** |
| Coloured shadow declarations | **153** across 145 selectors |
| `app/styles.css` parsed rules | **10,713** (baseline unchanged) |

⚠️ **The FA backlog is REPORTED, not auto-failed, and that is deliberate.**
Converting a site needs the real FontAwesome path data, which is not in this repo
and **cannot be fetched here — `cdnjs.cloudflare.com` is blocked by the sandbox's
egress policy** (`CONNECT tunnel failed, 403`). A policy denial is not retried or
routed around, and **path data is never invented** — a fabricated path draws
garbage. So icons convert phase by phase as the surface is rebuilt, and the
guard's `--strict` flag turns on once the count reaches zero.
⚠️ **The 153 coloured shadows are DECLARATIONS, not computed styles.** Some are
already dead, overridden later in the cascade (the round-509 notification badges,
styles.css ~19270, set `box-shadow:0 0 0 2.5px var(--ring)` + `animation:none`
over theirs). A dead declaration is still debt: the next person to edit that rule
re-enables a glow without meaning to. **Retire them, do not just override.**
⚠️ **Both guards had a fake-green bug, and both are worth remembering.**
`check-icons` reported **5** FA sites where there are 322: an alternation
`/fa-(?:solid)\s+fa-(\w+)|class="fa-(\w+)/` matched the *second* branch first at
the earlier position on `class="fa-solid fa-flag"`, captured `"solid"`, consumed
it, and never saw the real glyph. `check-tokens` reported **0** glows: its pattern
demanded three `\d+px` values, and offsets are routinely written unitless
(`0 0 12px …`). **A checker that reports zero is the one to distrust first** —
prove it against a known-positive before believing it.
- **Nothing was deleted.** `.fs2-*` is already gone; `.pk-*`'s three "matches" are
  comment mentions of the lost mockup (styles.css 1775, 4442, 10309), not rules;
  `.sc-*` (154 CSS / 179 JS) is the old staff console and belongs to its own
  phase. Deleting it here would have been a rename-and-pray with no way to verify.
- **No cache-bust bump on this one, on purpose.** Phase 0 ships nothing to a
  browser — the only runtime edit is the `FTZ_KIT`-gated route. Bumping would make
  every returning user re-download `app.js` (4.5 MB) + `styles.css` (1.5 MB) to
  receive zero changes, and **rule #1 is egress**.
